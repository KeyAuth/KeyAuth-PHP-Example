...
<head>
    <title>KeyAuth - Forgot Password for <?= $name; ?></title>
    <meta charset="UTF-8">
...
<body>
    ...
    <section class="flex items-center justify-center min-h-screen relative z-10">
        <div class="bg-custom-back rounded-xl p-4 md:p-10 w-full max-w-lg flex flex-col justify-center mx-2">
            <h1 class="mb-6 text-2xl md:text-3xl text-center font-semibold text-white">Reset Password</h1>
            <h3 class="mb-8 text-sm md:text-lg text-center font-normal text-gray-300">
                Remember your password? <a href="../" class="text-blue-500 hover:underline">Sign in</a>
            </h3>
            ...
            <form class="space-y-6" method="post">
                ...
                <div class="relative mt-6">
                    <input type="email" name="email" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-white bg-transparent rounded-lg border-1 border-gray-700 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                ...
                <button name="forgot" class="mt-6 w-full px-5 py-3 text-base font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 transition-colors">Reset Password</button>
            </form>
        </div>
    </section>
    ...
    if (isset($_POST['forgot'])) {
        if ($KeyAuthApp->forgot($_POST['username'], $_POST['email'])) {
            $KeyAuthApp->success("Password reset request sent!");
        }
    }
    ?>
</body>
